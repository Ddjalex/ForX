...
-- Note: MySQL requires a slightly different delete syntax for subqueries if targeting the same table
DELETE FROM prices WHERE market_id IN (SELECT id FROM (SELECT id FROM markets) as tmp);
DELETE FROM markets;
...
ON DUPLICATE KEY UPDATE 
    ...
    symbol_api = VALUES(symbol_api),
    symbol_tradingview = VALUES(symbol_tradingview),
...
-- Note: MySQL requires subquery for multi-row inserts from same table if using ON DUPLICATE KEY
